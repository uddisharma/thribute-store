...
import { Empty, SearchNotFoundIcon } from '@/component/ui/empty';
import {
  PiFileTextDuotone,
...
export default function SearchList({ onClose }: { onClose?: () => void }) {
  ...
  if (searchText.length > 0) {
    menuItemsFiltered = pageLinks.filter((item: any) => {
      ...
      return (
        label?.match(new RegExp(searchText?.toLowerCase())) ||
        (label?.toLowerCase().includes(searchText?.toLowerCase()) && label)
      );
    });
  }
