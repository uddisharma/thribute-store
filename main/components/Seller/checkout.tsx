...
const Checkout = () => {
  ...
  useEffect(() => {
    const dimensions =
      ...
      calculateTotalMetrics(cartState?.cartArray);
  ...
  const onSubmit = () => {
    ...
    const ourOrder = {
      ...
      charge: Math.ceil(calculateCharge(finalAmount)),
      ...
      axios
        ...
        .then((res) => {
          if (res.data?.status == "SUCCESS") {
            ...
            const orderedProduct = cartState?.cartArray?.map((e) => {
              return {
                id: e?.id,
                quantity: e?.quantity,
                name: e?.name,
  ...
  if (auth == false) {
    return router.push(`/login?ref=${params?.seller}/checkout`);
  }
                    ...
                    <div className="list-payment ">
                      <div
                        className={`type bg-surface p-5 border border-line rounded-lg ${
                          activetab === "shipping-address" ? "open" : ""
                        }`}
                      >
                        <input
                      ...
                      <div
                        className={`type bg-surface p-5 border border-line rounded-lg mt-5 ${
                          activetab === "shipping-service" ? "open" : ""
                        }`}
                      >
                        <input
                          className="cursor-pointer"
