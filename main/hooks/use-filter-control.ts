...
export function useFilterControls<StateType, ActionType>(
    ...
    (key: string) =>
      router.push(
        `${pathname}?${createQueryString(
          ["sort"],
          [isEqual(searchParams.get("sort"), key) ? `-${key}` : key]
        )}
      `
      ),
  ...
  const onSizeChange = useCallback(
    (pageNumber: string | number) =>
      router.push(
        `${pathname}?${createQueryString("size", pageNumber.toString())}`
      ),
    [createQueryString, pathname, router]
  );
  ...
  const applyFilter = useCallback(
    (key: string, payload: string | any[]) => {
      ...
      var url = new URL(`${pathname}?${params}`, window.location.origin);
      url.searchParams.delete("page");
      var updatedURL = url.toString();
      return router.push(updatedURL);
      // return router.push(`${pathname}?${params}`);
    },
    [createQueryString, pathname, router]
  );

  // const applyFilter1 = useCallback(
  //   (key: string, payload: string | any[]) => {
  //     const params = createQueryString(key, payload);
  //     var url = new URL(`${pathname}?${params}`, window.location.origin);
  //     var updatedURL = url.toString();
  ...
  const applyFilter1 = useCallback(
    (key: string, payload: string | any[]) => {
      ...
      var updatedURL = url.toString();
      console.log(updatedURL);
      return router.replace(updatedURL);
    },
    [createQueryString, pathname, router]
  );
